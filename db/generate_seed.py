...
# /v1/tags/attractions 실제 ID 사용 (2026-07-13 확인)
attractions = {
    10508: "재택근무", 10509: "유연근무", 10431: "스톡옵션", 10444: "자율복장",
    10462: "워크샵", 10427: "복지포인트", 10433: "자기계발지원", 10437: "식대지원",
    10436: "커피·스낵바", 10432: "임직원할인", 10435: "해외연수", 10441: "수평적조직",
    10420: "유급휴가", 10415: "주4일근무", 10430: "대출지원", 10426: "보너스",
    10438: "휴가비", 10425: "의료비지원",
}
attraction_ids = list(attractions)
# 규모급(eb)별로 더 어울리는 태그 가중치 — 스타트업은 자율·복지, 대형은 안정·지원 위주
attraction_pool = {
    0: [10508, 10509, 10431, 10444, 10462, 10433, 10420, 10415, 10441],   # 스타트업
    1: [10508, 10509, 10427, 10433, 10437, 10436, 10420, 10426, 10441],   # 중견급
    2: [10427, 10437, 10436, 10432, 10435, 10420, 10430, 10426, 10438, 10425],  # 대형급
}
...
for i, (name, ind, eb, sb, lb, loc) in enumerate(company_defs, start=1):
    ...
    pool = attraction_pool[eb]
    n_tags = random.randint(3, 5)
    for tid in random.sample(pool, min(n_tags, len(pool))):
        ct_rows.append(f"({i}, {tid})")
...
out.append("INSERT INTO company_tags (company_id, tag_id) VALUES\n" + ",\n".join(ct_rows) + ";")
