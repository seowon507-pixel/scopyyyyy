...
> v2.1부터 공고 탐색·북마크·자소서 추천은 원티드 실 API 데이터를 쓴다 (시장 개요·기업 비교는 계속 가상 데이터 — 이유는 prd.md §3 참고).
...
1. **시장 개요** — 신규 공고·채용 기업·평균 보상금·신입 가능 KPI + 추이/직군/스킬/경력대 차트 (가상 데이터)
...
db/fetch_live_jobs.py   원티드 실 API에서 활성 공고를 가져오는 스크립트 (.env 필요)
