...
(() => {
  ...
  function downloadGeneratedResume() {
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><style>
      @page{size:A4;margin:18mm} body{font-family:'Malgun Gothic',sans-serif;color:#222;line-height:1.65;margin:0}
      header{border-bottom:3px solid #222;padding-bottom:15px;margin-bottom:24px}header span{font-size:10pt;color:#666}h1{font-size:26px;margin:5px 0}header p{margin:0;color:#555}
      h2{font-size:15px;margin:24px 0 9px;border-left:4px solid #555;padding-left:8px}p,li{font-size:10.5pt}ul{padding-left:20px}.activity-items li{margin-bottom:8px}.activity-items span{display:block;color:#666;font-size:9.5pt}
      </style></head><body>${resumeBodyHtml(model)}</body></html>`;
    const blob = new Blob(["\ufeff", html], { type: "application/msword;charset=utf-8" });
    const objectUrl = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = objectUrl;
    link.download = `scopy_${model.template.title}_이력서_${model.name}.doc`.replace(/[\\/:*?"<>|]/g, "_");
